...
\verb@Rmpi@, in advance. \verb@parallel@ is necessary for all operating systems. \verb@Rmpi@ is optional, it is needed when you are working on grid engine which has MPI software installed, and wish to accelerate the analysis.
\begin{description}
  \item[parallel] \hfill \\
...
As long as your R library has "parallel" and "Rmpi"(optional), you are ready for \verb@CORE@ 
...
using \verb@"mpi.ge"@. Run the example below only when you installed \verb@Rmpi@ and grid engine environment is available.(delete the \# when 
you run the example)
<<\verb@"mpi.ge"@>>=
data(testInputCORE)
data(testInputBoundaries)
